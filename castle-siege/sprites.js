...
const SpriteLib = (function () {
  ...
  function drawEmberPatch(g, R) {
    for (let i = 0; i < 7; i++) {
      ...
      glow(g, x, y, 6 + R() * 6, '#ff6a2a', 0.4);
      g.fillStyle = rgba(255, 130 + R() * 80, 50, 0.9);
      g.beginPath(); g.arc(x, y, 1.4 + R() * 1.4, 0, Math.PI * 2); g.fill();
    }
    g.strokeStyle = 'rgba(255,120,50,0.7)'; g.lineWidth = 1.4;
    for (let i = 0; i < 3; i++) {
  ...
  function drawCastle(g, T, theme, cx, cy, R) {
    g.save();
    g.translate(cx, cy);
    const stone = '#b0a8b8', stoneD = sh('#b0a8b8', 0.72), stoneL = lt('#b0a8b8', 0.3);
    const roof = '#3e63b0', roofD = sh('#3e63b0', 0.7), roofL = lt('#3e63b0', 0.3);
    const gold = '#e8c34a';
